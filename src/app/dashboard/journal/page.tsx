...
import { ChevronLeft, ChevronRight, BookOpen, Paperclip, Trash2 } from "lucide-react";
...
export default function JournalPage() {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 pt-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={goToday} className="flex flex-col items-center">
            <span
              className={cn(
                "text-lg font-bold",
                todayCheck ? "text-accent" : "text-zinc-900 dark:text-white"
              )}
            >
              {todayCheck ? "Today" : format(date, "EEE, MMM d")}
            </span>
            {!todayCheck && (
              <span className="text-[10px] text-accent">Tap for today</span>
            )}
          </button>
          <button
            onClick={goForward}
      ...
      <Card className="p-4 mb-4">
          ...
          <div className="flex items-center gap-1.5">
            {(content || mood) && (
              <button
                onClick={handleClear}
                className={cn(
                  "flex h-8 items-center gap-1 rounded-lg px-2 text-xs font-medium transition-all",
                  confirmClear === 0
        ...
        <textarea
          ...
          onInput={(e) => {
            ...
            t.style.height = "auto";
            t.style.height =
              Math.max(t.scrollHeight, window.innerHeight * 0.5) + "px";
          }}
        />
      ...
      {entries.length > 0 && (
        <div>
          <p className="mb-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider px-1">
            Past Entries
          </p>
      ...
      {!isLoading && entries.length === 0 && !currentEntry && (
        <div className="flex flex-col items-center pt-8 animate-fade-in">
          ...
          <p className="text-sm text-zinc-500">Start writing your thoughts for today</p>
        </div>
      )}
    </div>
  );
